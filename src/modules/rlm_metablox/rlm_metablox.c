...
static const CONF_PARSER module_config[] = {
	{ "integer", FR_CONF_OFFSET(PW_TYPE_INTEGER, rlm_metablox_t, value), "1" },
	{ "boolean", FR_CONF_OFFSET(PW_TYPE_BOOLEAN, rlm_metablox_t, boolean), "no" },
	{ "string", FR_CONF_OFFSET(PW_TYPE_STRING, rlm_metablox_t, string), NULL },
	{ "ipaddr", FR_CONF_OFFSET(PW_TYPE_IPV4_ADDR, rlm_metablox_t, ipaddr), "*" },
	CONF_PARSER_TERMINATOR
};

static int rlm_metablox_cmp(UNUSED void *instance, REQUEST *request, UNUSED VALUE_PAIR *thing, VALUE_PAIR *check,
...
static int mod_instantiate(CONF_SECTION *conf, void *instance)
{
	...
	if (dict_addattr("Metablox-Paircmp", -1, 0, PW_TYPE_STRING, flags) < 0) {
		ERROR("Failed creating paircmp attribute: %s", fr_strerror());

		return -1;
	}

	paircompare_register(dict_attrbyname("Metablox-Paircmp"), dict_attrbyvalue(PW_USER_NAME, 0), false,
			     rlm_metablox_cmp, inst);
 ...
 *	is single-threaded.
 */
extern module_t rlm_metablox;
module_t rlm_metablox = {
	.magic		= RLM_MODULE_INIT,
	.name		= "metablox",
	.type		= RLM_TYPE_THREAD_SAFE,
	.inst_size	= sizeof(rlm_metablox_t),
